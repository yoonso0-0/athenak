...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  grids.push_back(std::make_unique<SphericalGrid>(pmbp, nlev_sphere, 64.0));
  grids.push_back(std::make_unique<SphericalGrid>(pmbp, nlev_sphere, 128.0));
  grids.push_back(std::make_unique<SphericalGrid>(pmbp, nlev_sphere, 256.0));
  grids.push_back(std::make_unique<SphericalGrid>(pmbp, nlev_sphere, 512.0));
  grids.push_back(std::make_unique<SphericalGrid>(pmbp, nlev_sphere, 1024.0));
      ...
      KOKKOS_LAMBDA(const int &idx) {
        ...
        for (int g = 0; g < nradii; ++g) {
          if (r_ks < d_radii(g)) {
            ...
            // T^0_i
            Kokkos::atomic_add(&d_sum(g, 6),
                               vol * (wtot * u0 * u_1 - b0 * b_1));
            Kokkos::atomic_add(&d_sum(g, 7),
                               vol * (wtot * u0 * u_2 - b0 * b_2));
            Kokkos::atomic_add(&d_sum(g, 8),
                               vol * (wtot * u0 * u_3 - b0 * b_3));
            ...
            if (is_mhd) {
              ...
              // T^0_i (EM part only)
              Kokkos::atomic_add(&d_sum(g, 12),
                                 vol * (b_sq * u0 * u_1 - b0 * b_1));
              Kokkos::atomic_add(&d_sum(g, 13),
                                 vol * (b_sq * u0 * u_2 - b0 * b_2));
              Kokkos::atomic_add(&d_sum(g, 14),
                                 vol * (b_sq * u0 * u_3 - b0 * b_3));
            }
          }
        }
      });
