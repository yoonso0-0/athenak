...
TaskStatus MHD::RescaleAndAddRecoil(ParameterInput *pin) {
  ...
  par_for(
      ...
      is, ie, KOKKOS_LAMBDA(int m, int k, int j, int i) {
        ...
        bcc_(m, IBX, k, j, i) *= sqrt(rho0_over_rho_infty / a_over_rg);
        bcc_(m, IBY, k, j, i) *= sqrt(rho0_over_rho_infty / a_over_rg);
        bcc_(m, IBZ, k, j, i) *= sqrt(rho0_over_rho_infty / a_over_rg);

        // Face-centered magnetic fields
        b0.x1f(m, k, j, i) *= sqrt(rho0_over_rho_infty / a_over_rg);
        b0.x2f(m, k, j, i) *= sqrt(rho0_over_rho_infty / a_over_rg);
        b0.x3f(m, k, j, i) *= sqrt(rho0_over_rho_infty / a_over_rg);
        // Include extra face-component at edges of the meshblock
        if (i == ie) {
          b0.x1f(m, k, j, i + 1) *= sqrt(rho0_over_rho_infty / a_over_rg);
        }
        if (j == je) {
          b0.x2f(m, k, j + 1, i) *= sqrt(rho0_over_rho_infty / a_over_rg);
        }
        if (k == ke) {
          b0.x3f(m, k + 1, j, i) *= sqrt(rho0_over_rho_infty / a_over_rg);
        }
