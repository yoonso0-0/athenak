...
void RefinementCondition(MeshBlockPack *pmbp) {
  ...
  par_for_outer("StaticCondition", DevExeSpace(), 0, 0, 0, nmb - 1,
    KOKKOS_LAMBDA(TeamMember_t tmember, const int m) {
      // Get block bounds
      Real x2min = size.d_view(m).x2min;
      Real x2max = size.d_view(m).x2max;

      // Approximate block center
      Real x2v_center = 0.5 * (x2min + x2max);

      if (x2v_center > 0.5) {refine_flag_.d_view(m+mbs) = 1;} 
      if (x2v_center < 0.5) {refine_flag_.d_view(m+mbs) = 0;}
    }
  );
}
